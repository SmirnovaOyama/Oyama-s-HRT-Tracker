...
**问题定位**：不是解析核、$V_d$ 或 $k_3$ 的问题，而是 6.2 的口腔模型把血浆侧的 $k_{\text{SL}}=1.8\ \mathrm{h}^{-1}$ 直接当成了口腔内的黏膜渗透常数。二者物理意义不同（见上），而 1.8 h⁻¹ 对口腔而言快了约 5 倍，于是「黏膜吸收 vs 吞咽清除」这场竞争被模型判给了黏膜。把该常数单独重新拟合为 $k_{\text{perm}}=0.37\ \mathrm{h}^{-1}$（令 10 min 档命中 Doll 的实测峰值），即得上表；模型其余部分一律未动，$T_{\max}$ 仍在 1 h 左右。
